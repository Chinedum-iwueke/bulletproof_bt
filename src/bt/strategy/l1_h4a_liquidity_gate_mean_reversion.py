"""L1-H4A Liquidity-uncertainty-gated mean reversion strategy."""
...
from bt.hypotheses.l1_h4a import RollingQuantileGate, bars_for_30_calendar_days, spread_proxy_from_bar
...
@register_strategy("l1_h4a_liquidity_gate_mean_reversion")
class L1H4ALiquidityGateMeanReversionStrategy(Strategy):
    def __init__(
        self,
        *,
        timeframe: str = "5m",
    ...
    def _state_for(self, symbol: str) -> _State:
        if symbol not in self._state:
            self._state[symbol] = _State(
                ...
                gate=RollingQuantileGate(bars_for_30_calendar_days(self._timeframe), q=self._q_liq),
            )
    ...
    def on_bars(self, ts: pd.Timestamp, bars_by_symbol: dict[str, Bar], tradeable: set[str], ctx: Mapping[str, Any]) -> list[Signal]:
        ...
        if not isinstance(htf_root, Mapping):
            raise RuntimeError(f"L1-H4A requires ctx['htf']['{self._timeframe}'] for two-clock semantics.")
        htf_for_tf = htf_root.get(self._timeframe)
        if htf_for_tf is None:
            htf_for_tf = {}
        if not isinstance(htf_for_tf, Mapping):
            raise RuntimeError(f"L1-H4A requires mapping ctx['htf']['{self._timeframe}'] for two-clock semantics.")
        ...
        for symbol in sorted(tradeable):
            ...
            if z_vwap_t <= -self._z0:
                side = Side.BUY
                entry_reason = "liquidity_gate_fade_long"
            elif z_vwap_t >= self._z0:
                side = Side.SELL
                entry_reason = "liquidity_gate_fade_short"
            if side is None:
                continue
