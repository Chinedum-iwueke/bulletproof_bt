"""L1-H3A HAR-gated trend continuation strategy (L1-H1 directional baseline)."""
...
@register_strategy("l1_h3_har_rv_gate_trend")
class L1H3HarRVGateTrendStrategy(Strategy):
    def __init__(
        self,
        *,
        timeframe: str = "15m",
    ...
    def on_bars(self, ts: pd.Timestamp, bars_by_symbol: dict[str, Bar], tradeable: set[str], ctx: Mapping[str, Any]) -> list[Signal]:
        ...
        if not isinstance(htf_root, Mapping):
            raise RuntimeError(f"L1-H3 requires ctx['htf']['{self._timeframe}'] for two-clock semantics.")
        htf_for_tf = htf_root.get(self._timeframe)
        if htf_for_tf is None:
            htf_for_tf = {}
        if not isinstance(htf_for_tf, Mapping):
            raise RuntimeError(f"L1-H3 requires mapping ctx['htf']['{self._timeframe}'] for two-clock semantics.")
