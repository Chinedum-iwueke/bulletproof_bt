"""L1-H6A Vol-of-vol-gated mean reversion strategy."""
...
from bt.hypotheses.l1_h6a import RollingQuantileGate, RollingStd, bars_for_30_calendar_days, wvov_hours_to_bars
...
@register_strategy("l1_h6a_vov_gate_mean_reversion")
class L1H6AVovGateMeanReversionStrategy(Strategy):
    def __init__(
        self,
        *,
        timeframe: str = "5m",
        ...
        size_adjustment_enabled: bool = False,
        vov_ref: float = 0.005,
        cap_multiplier: float = 1.5,
    ) -> None:
        self._timeframe = timeframe
        self._wvov_hours = int(Wvov_hours)
        self._q_vov = float(q_vov)
        self._z0 = float(z0)
        ...
        self._size_adjustment_enabled = bool(size_adjustment_enabled)
        self._vov_ref = float(vov_ref)
    ...
    def _state_for(self, symbol: str) -> _State:
        if symbol not in self._state:
            self._state[symbol] = _State(
                ...
                vov_std=RollingStd(wvov_hours_to_bars(timeframe=self._timeframe, wvov_hours=self._wvov_hours)),
                gate=RollingQuantileGate(bars_for_30_calendar_days(self._timeframe), q=self._q_vov),
            )
    ...
    def on_bars(self, ts: pd.Timestamp, bars_by_symbol: dict[str, Bar], tradeable: set[str], ctx: Mapping[str, Any]) -> list[Signal]:
        ...
        if not isinstance(htf_root, Mapping):
            raise RuntimeError(f"L1-H6A requires ctx['htf']['{self._timeframe}'] for two-clock semantics.")
        htf_for_tf = htf_root.get(self._timeframe)
        if htf_for_tf is None:
            htf_for_tf = {}
        if not isinstance(htf_for_tf, Mapping):
            raise RuntimeError(f"L1-H6A requires mapping ctx['htf']['{self._timeframe}'] for two-clock semantics.")
        ...
        for symbol in sorted(tradeable):
            ...
            vov_t = st.vov_std.update(rv_t) if has_new_signal_bar else None
            q_threshold_t, vov_gate_t = st.gate.update(vov_t) if has_new_signal_bar else (None, None)
            ...
            if current is not None:
                ...
                if has_new_signal_bar:
                    ...
                    if st.signal_bars_held >= self._t_hold:
                        signals.append(Signal(ts=ts, symbol=symbol, side=Side.SELL if current == Side.BUY else Side.BUY, signal_type="l1_h6a_exit", confidence=1.0, metadata={"close_only": True, "exit_reason": "time_stop", "signal_bars_held": st.signal_bars_held, "hold_time_unit": "signal_bars", "signal_timeframe": self._timeframe}))
                        self._clear_position_state(st)
                        continue

                if st.stop_price_frozen is not None:
                    if current == Side.BUY and bar.low <= st.stop_price_frozen:
                        signals.append(Signal(ts=ts, symbol=symbol, side=Side.SELL, signal_type="l1_h6a_exit", confidence=1.0, metadata={"close_only": True, "exit_reason": "atr_stop", "stop_price": st.stop_price_frozen, "stop_distance": st.stop_distance_frozen, "atr_entry": st.atr_entry, "exit_monitoring_timeframe": "1m"}))
                        self._clear_position_state(st)
                        continue
                    if current == Side.SELL and bar.high >= st.stop_price_frozen:
                        signals.append(Signal(ts=ts, symbol=symbol, side=Side.BUY, signal_type="l1_h6a_exit", confidence=1.0, metadata={"close_only": True, "exit_reason": "atr_stop", "stop_price": st.stop_price_frozen, "stop_distance": st.stop_distance_frozen, "atr_entry": st.atr_entry, "exit_monitoring_timeframe": "1m"}))
                        self._clear_position_state(st)
                        continue
                ...
                if active_base_vwap is not None:
                    if current == Side.BUY and bar.close >= active_base_vwap:
                        signals.append(Signal(ts=ts, symbol=symbol, side=Side.SELL, signal_type="l1_h6a_exit", confidence=1.0, metadata={"close_only": True, "exit_reason": "vwap_touch", "vwap_t": active_base_vwap, "vwap_mode": "session", "exit_monitoring_timeframe": "1m"}))
                        self._clear_position_state(st)
                        continue
                    if current == Side.SELL and bar.close <= active_base_vwap:
                        signals.append(Signal(ts=ts, symbol=symbol, side=Side.BUY, signal_type="l1_h6a_exit", confidence=1.0, metadata={"close_only": True, "exit_reason": "vwap_touch", "vwap_t": active_base_vwap, "vwap_mode": "session", "exit_monitoring_timeframe": "1m"}))
                        self._clear_position_state(st)
                        continue
                continue

            self._clear_position_state(st)
            if not has_new_signal_bar or vov_gate_t is not True:
                continue
            ...
            if z_vwap_t <= -self._z0:
                side = Side.BUY
                entry_reason = "vov_gate_fade_long"
            elif z_vwap_t >= self._z0:
                side = Side.SELL
                entry_reason = "vov_gate_fade_short"
            if side is None:
                continue
            ...
            signals.append(
                Signal(
                    ...
                    symbol=symbol,
                    side=side,
                    signal_type="l1_h6a_vov_gate_mean_reversion",
                    confidence=1.0,
                    metadata={
                        ...
                        "rv_t": rv_t,
                        "vov_t": vov_t,
                        "Wvov_hours": self._wvov_hours,
                        "q_vov": self._q_vov,
                        "q_threshold_t": q_threshold_t,
                        "vov_gate_t": vov_gate_t,
                        ...
                        "size_adjustment_enabled": self._size_adjustment_enabled,
                        "vov_ref": self._vov_ref,
