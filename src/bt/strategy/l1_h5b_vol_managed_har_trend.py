        ...
        for symbol in sorted(tradeable):
            ...
            if current is not None:
                ...
                if st.signal_bars_held >= self._t_hold and has_new_signal_bar:
                    signals.append(Signal(ts=ts, symbol=symbol, side=Side.SELL if current == Side.BUY else Side.BUY, signal_type="l1_h5b_exit", confidence=1.0, metadata={
                        "decision_trace": make_decision_trace(
                            reason_code="vol_managed_har_trend_entry",
                            setup_class="vol_managed_har_trend",
                            hypothesis_branch="entry",
                            conditions_bool_map={},
                            blockers_bool_map={},
                            permission_layer_state={},
                            parameter_combination={"strategy": "l1_h5b_vol_managed_har_trend"},
                            gate_values={},
                            gate_thresholds={},
                            gate_margins={},
                ...
                if st.stop_price_frozen is not None:
                    if current == Side.BUY and bar.low <= st.stop_price_frozen:
                        signals.append(Signal(ts=ts, symbol=symbol, side=Side.SELL, signal_type="l1_h5b_exit", confidence=1.0, metadata={
                        "decision_trace": make_decision_trace(
                            reason_code="vol_managed_har_trend_entry",
                            setup_class="vol_managed_har_trend",
                            hypothesis_branch="entry",
                    ...
                    if current == Side.SELL and bar.high >= st.stop_price_frozen:
                        signals.append(Signal(ts=ts, symbol=symbol, side=Side.BUY, signal_type="l1_h5b_exit", confidence=1.0, metadata={
                        "decision_trace": make_decision_trace(
                            reason_code="vol_managed_har_trend_entry",
                            setup_class="vol_managed_har_trend",
                            hypothesis_branch="entry",
