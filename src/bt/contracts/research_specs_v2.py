...
def validate_hypothesis_card(
    card: dict[str, Any], *, require_confirmed: bool = True
) -> list[str]:
    errors: list[str] = []
    required = (
        "card_id",
    ...
    registered_contract = (
        card.get("representation_mode") == "registered_engine_contract"
    )
    ...
    elif not card.get("engine_hypothesis_template") or not card.get(
        "engine_hypothesis_template_digest"
    ):
    ...
    if require_confirmed and (
        not card.get("confirmed_by") or not card.get("confirmed_at")
    ):
        errors.append("card_confirmation_identity_required")
    for dataset in (
        card.get("data_requirements", [])
        if isinstance(card.get("data_requirements"), list)
        else []
    ):
        if dataset not in DATASETS:
            errors.append(f"card_dataset_unsupported_{dataset}")
    semantics = (
        card.get("execution_semantics")
        if isinstance(card.get("execution_semantics"), dict)
        else {}
    )
    ...
    for index, feature in enumerate(
        card.get("features", []) if isinstance(card.get("features"), list) else []
    ):
        if (
            not isinstance(feature, dict)
            or not feature.get("id")
            or not feature.get("source")
            or not feature.get("transform")
        ):
    ...
    for index, gate in enumerate(
        card.get("gates", []) if isinstance(card.get("gates"), list) else []
    ):
        if not isinstance(gate, dict) or gate.get("op") not in PORTABLE_OPS:
            errors.append(f"card_gate_{index}_invalid")
    provenance = (
        card.get("field_provenance")
        if isinstance(card.get("field_provenance"), dict)
        else {}
    )
    for field in ("claim", "entry", "exit"):
        state = (
            provenance.get(field, {}).get("state")
            if isinstance(provenance.get(field), dict)
            else None
        )
        if require_confirmed and state in {
            "unresolved",
            "unsupported",
            "inferred",
            "recommended",
...
def normalize_card(card: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized = {
        ...
        "title": card["title"],
        "claim": card["claim"],
        "mechanism": card["market_mechanism"],
        "engine_strategy_name": card.get("engine_strategy_name"),
        "engine_hypothesis_template": card.get("engine_hypothesis_template"),
        "engine_hypothesis_template_digest": card.get(
            "engine_hypothesis_template_digest"
        ),
        "feature_graph": sorted(
            deepcopy(card["features"]), key=lambda item: str(item["id"])
        ),
        "gate_graph": deepcopy(card["gates"]),
        "entry": deepcopy(card["entry"]),
        "exit": deepcopy(card["exit"]),
        "sizing": deepcopy(card["sizing"]),
        "risk_controls": deepcopy(card["risk_controls"]),
        "parameters": {
            key: deepcopy(card["parameters"][key]) for key in sorted(card["parameters"])
        },
        "data_requirements": sorted(set(card["data_requirements"])),
        "logging_requirements": sorted(set(card["logging_requirements"])),
        "evaluation": deepcopy(card["evaluation"]),
        "falsification_criteria": deepcopy(card["falsification_criteria"]),
        "expected_failure_modes": deepcopy(card["expected_failure_modes"]),
        "execution_semantics": deepcopy(card["execution_semantics"]),
        "source_citations": deepcopy(card.get("source_citations", [])),
        "field_provenance": deepcopy(card["field_provenance"]),
    }
    return normalized
...
def build_hypothesis_spec_v2(ir: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "hypothesis_id": ir["card_id"],
        "program_id": ir["program_id"],
        "title": ir["title"],
        "claim": ir["claim"],
        "market_mechanism": ir["mechanism"],
        "observable_features": [item["id"] for item in ir["feature_graph"]],
        "required_datasets": ir["data_requirements"],
        "invalidation_criteria": ir["falsification_criteria"],
        "expected_failure_modes": ir["expected_failure_modes"],
        "parameter_grid": ir["parameters"],
        "evaluation": ir["evaluation"],
        "execution_semantics": ir["execution_semantics"],
        "source_card_hash": ir["card_hash"],
        "source_citations": ir["source_citations"],
        "field_provenance": ir["field_provenance"],
    }


def build_engine_hypothesis_yaml(
    ir: dict[str, Any], *, repo_root: str | Path = "."
) -> dict[str, Any]:
    ...
    if template:
        ...
        if (
            ...
            and sha256(path.read_bytes()).hexdigest() != expected_template_digest
        ):
            raise ResearchSpecV2Error(["engine_hypothesis_template_digest_mismatch"])
        template_strategy = (
            payload.get("entry", {}).get("strategy")
        ...
        template_grid = (
            ...
            if isinstance(payload.get("parameter_grid"), dict)
            else {}
        )
        mismatched = [
            key
            for key, values in ir["parameters"].items()
            if key not in template_grid or template_grid[key] != values
        ]
        if mismatched:
            raise ResearchSpecV2Error(
                [
                    f"engine_hypothesis_template_parameter_mismatch_{key}"
                    for key in mismatched
                ]
            )
        template_semantics = payload.get("execution_semantics", {})
        if not isinstance(template_semantics, dict):
            raise ResearchSpecV2Error(["engine_hypothesis_template_semantics_invalid"])
        payload["execution_semantics"] = {
            **template_semantics,
            **deepcopy(ir["execution_semantics"]),
        }
    else:
        ...
        payload = {
            "hypothesis_id": ir["card_id"],
            "title": ir["title"],
            "description": ir["claim"],
            "research_layer": "generated",
            "hypothesis_family": "portable_research_graph",
            "version": "1.0.0",
            "required_indicators": [],
            "indicator_defaults": {},
            "parameter_grid": ir["parameters"],
            ...
            "evaluation": {
                **deepcopy(ir["evaluation"]),
                "required_tiers": ir["evaluation"].get("tiers", ["Tier2", "Tier3"]),
            },
            "logging": {
                "schema_version": "1.0",
                "required_fields": ir["logging_requirements"],
            },
            "runtime_controls": {
                "enabled": True,
                "max_variants": 128,
                "tags": ["generated", "confirmed_card"],
            },
            "notes": {
                "falsification_criteria": ir["falsification_criteria"],
                "failure_modes": ir["expected_failure_modes"],
            },
        }
    payload["truth_contract"] = deepcopy(ENGINE_TRUTH_BLOCK)
    payload["generation_provenance"] = {
        "schema_version": ENGINE_YAML_VERSION,
        "source_card_hash": ir["card_hash"],
...
def build_strategy_spec_v2(ir: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "strategy_spec_id": f"strategy-{ir['card_id']}",
        "program_id": ir["program_id"],
        "hypothesis_id": ir["card_id"],
        "engine_strategy_name": ir.get("engine_strategy_name"),
        "feature_graph": ir["feature_graph"],
        "gate_graph": ir["gate_graph"],
        "entry": ir["entry"],
        "exit_state_machine": ir["exit"],
        "sizing": ir["sizing"],
        "risk_policy": ir["risk_controls"],
        "parameter_grid": ir["parameters"],
        "parameter_defaults": _default_parameters(ir["parameters"]),
        "data_requirements": ir["data_requirements"],
        "logging_contract": ir["logging_requirements"],
        "evaluation_contract": ir["evaluation"],
        "falsification_contract": ir["falsification_criteria"],
        "execution_semantics": ir["execution_semantics"],
...
def compile_readiness(
    spec: dict[str, Any], *, available_datasets: set[str] | None = None
) -> dict[str, Any]:
    blockers: list[dict[str, str]] = []
    available = (
        available_datasets
        if available_datasets is not None
        else set(spec.get("data_requirements", []))
    )
    missing_data = sorted(set(spec.get("data_requirements", [])) - available)
    if missing_data:
        blockers.extend(
            {"code": "dataset_missing", "detail": item} for item in missing_data
        )
        status = "data_blocked"
    else:
        semantics = spec.get("execution_semantics", {})
        bad_semantics = [
            key for key, value in EXACT_TRUTH.items() if semantics.get(key) != value
        ]
        if bad_semantics:
            blockers.extend(
                {"code": "semantic_mismatch", "detail": item} for item in bad_semantics
            )
            ...
            unsupported_features = [
                item.get("transform")
                for item in spec.get("feature_graph", [])
                if item.get("transform") not in PORTABLE_TRANSFORMS
            ]
            unsupported_gates = [
                item.get("op")
                for item in spec.get("gate_graph", [])
                if item.get("op") not in PORTABLE_OPS
            ]
            feature_ids = {
                str(item.get("id")) for item in spec.get("feature_graph", [])
            }
            parameter_ids = set(spec.get("parameter_grid", {}))
            invalid_gate_refs = [
                str(item.get("left", item.get("field")))
                for item in spec.get("gate_graph", [])
                if str(item.get("left", item.get("field"))) not in feature_ids
            ]
            invalid_gate_params = [
                str(item.get("right_param", item.get("param")))
                for item in spec.get("gate_graph", [])
                if item.get("right") is None
                and str(item.get("right_param", item.get("param"))) not in parameter_ids
            ]
            ...
            auxiliary_sources = sorted(
                {
                    str(item.get("source"))
                    for item in spec.get("feature_graph", [])
                    if item.get("source") not in {"ohlcv", "derived"}
                }
            )
            sizing = spec.get("sizing", {})
            sizing_supported = (
                sizing.get("mode") == "constant_r"
                and sizing.get("stop_required") is True
            )
            logging = set(spec.get("logging_contract", []))
            logging_supported = {"decision_trace", "stop_price"}.issubset(logging)
            if (
                unsupported_features
                or unsupported_gates
                or invalid_gate_refs
                or invalid_gate_params
                or auxiliary_sources
                or exit_type != "fixed_stop_time_exit"
                or not sizing_supported
                or not logging_supported
            ):
                blockers.extend(
                    {"code": "primitive_missing", "detail": str(item)}
                    for item in [*unsupported_features, *unsupported_gates]
                )
                blockers.extend(
                    {"code": "feature_reference_missing", "detail": item}
                    for item in invalid_gate_refs
                )
                blockers.extend(
                    {"code": "parameter_reference_missing", "detail": item}
                    for item in invalid_gate_params
                )
                blockers.extend(
                    {"code": "auxiliary_source_requires_implementation", "detail": item}
                    for item in auxiliary_sources
                )
                if exit_type != "fixed_stop_time_exit":
                    blockers.append(
                        {"code": "exit_not_portable", "detail": str(exit_type)}
                    )
                if not sizing_supported:
                    blockers.append(
                        {
                            "code": "sizing_not_portable",
                            "detail": str(sizing.get("mode")),
                        }
                    )
                if not logging_supported:
                    blockers.append(
                        {
                            "code": "logging_contract_incomplete",
                            "detail": "decision_trace,stop_price",
                        }
                    )
    ...
    return {
        ...
        "blockers": blockers,
        "capabilities": {
            "registered_strategy": spec.get("engine_strategy_name")
            in STRATEGY_REGISTRY,
            "portable_feature_graph": not any(
                item.get("transform") not in PORTABLE_TRANSFORMS
                for item in spec.get("feature_graph", [])
            ),
            "truth_contract_valid": not any(
                key
                for key, value in EXACT_TRUTH.items()
                if spec.get("execution_semantics", {}).get(key) != value
            ),
            "engine_owned_risk": spec.get("execution_semantics", {}).get(
                "risk_authority"
            )
            == "engine",
            "rich_logging": {"decision_trace", "stop_price"}.issubset(
                set(spec.get("logging_contract", []))
            ),
        },
        "compiler_version": COMPILER_VERSION,
        "source_card_hash": spec.get("source_card_hash"),
    }


def compile_strategy_spec_v2(
    spec: dict[str, Any], readiness: dict[str, Any]
) -> dict[str, Any]:
    if readiness.get("status") not in {"registry_ready", "graph_compilable"}:
        raise ResearchSpecV2Error(
            [f"strategy_not_compilable_{readiness.get('status')}"]
        )
    strategy_name = (
        spec.get("engine_strategy_name")
        if readiness["status"] == "registry_ready"
        else "research_graph_v1"
    )
    return {
        "schema_version": "run_config_from_strategy_spec_v2",
        ...
        "data": {
            ...
            if "research_panel" in spec["data_requirements"]
            else "ohlcv"
        },
        "required_datasets": spec["data_requirements"],
        "risk": spec["risk_policy"],
        "execution_semantics": spec["execution_semantics"],
        "truth_contract": spec["truth_contract"],
        "compiler": {"version": COMPILER_VERSION, "status": readiness["status"]},
    }
...
def build_implementation_task(
    ...
    return {
        "schema_version": "strategy_implementation_task_v1",
        "task_id": f"implementation-{spec['strategy_spec_id']}",
        "strategy_spec_id": spec["strategy_spec_id"],
        "strategy_spec_hash": canonical_hash(spec),
        "source_card_hash": spec["source_card_hash"],
        "status": "draft",
        ...
        "required_evidence": [
            ...
            "determinism_tests",
            "rich_logging_tests",
            "OHLCV_fallback_smoke",
            "enriched_data_smoke",
            "stable_membership_smoke",
            "volatile_membership_smoke",
            "classic_fast_parity_or_classic_only",
            "experiment_truth_PASS",
        ],
        "prohibited_shortcuts": [
            "future_values",
            "forward_aux_join",
            "interpolation",
            "strategy_owned_execution",
            "strategy_owned_risk",
            "uncertified_result_publication",
        ],
...
def build_artifact_bundle(
    ...
    *,
    ...
    bundle = {
        "schema_version": "research_spec_bundle_v2",
        "card": card,
        "normalized_ir": ir,
        "hypothesis_spec": hypothesis,
        "engine_hypothesis_yaml": engine_yaml,
...
def write_artifact_bundle(
    ...
    mapping = {
        "hypothesis_spec": "hypothesis_spec_v2.json",
        "strategy_spec": "strategy_spec_v2.json",
        "compile_readiness": "compile_readiness_report_v1.json",
        "normalized_ir": "research_strategy_ir_v1.json",
    }
    ...
    yaml_path.write_text(
        yaml.safe_dump(bundle["engine_hypothesis_yaml"], sort_keys=False),
        encoding="utf-8",
    )
    ...
    if "implementation_task" in bundle:
        ...
        path.write_text(
            json.dumps(bundle["implementation_task"], indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
...
def upgrade_hypothesis_spec_v1(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "compatibility": {
            "upgraded_from": "hypothesis_spec_v1",
            "execution_status": "requires_card_confirmation",
        },
    }
...
def upgrade_strategy_spec_v1(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "user_approval_required": True,
        "compatibility": {
            "upgraded_from": "strategy_spec_v1",
            "execution_status": "requires_card_confirmation",
        },
    }
