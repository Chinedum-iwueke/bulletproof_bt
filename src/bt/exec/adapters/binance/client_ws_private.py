...
class BinancePrivateWSClient:
    def __init__(
        ...
        *,
        ...
        product_type: str,
        enabled: bool = True,
        socket_factory: Callable[[str], WebSocketLike] | None = None,
    ) -> None:
        self._rest, self._ws_base_url, self._product_type, self._enabled = (
            rest,
            ws_base_url.rstrip("/"),
            product_type,
            enabled,
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._read_loop, name="binance-private-stream", daemon=True
        )
        self._thread.start()

    def _connect(self) -> None:
        response = self._rest.api_post(self._endpoint())
        if not isinstance(response.result, dict) or not response.result.get(
            "listenKey"
        ):
            ...
            try:
                ...
                if isinstance(payload, dict):
                    ...
                    self._messages.append(
                        BinancePrivateMessage(
                            now, str(payload.get("e", "unknown")), payload
                        )
                    )
    ...
    def stop(self) -> None:
        ...
        if self._listen_key:
            try:
                self._rest.api_delete(
                    self._endpoint(), params={"listenKey": self._listen_key}
                )
            except Exception:
                pass
        self._socket = None
    ...
    def ready(self) -> bool:
        return (
            self._enabled
            and self._running
            and self._listen_key is not None
    ...
    def push_test_message(self, payload: dict[str, Any]) -> None:
        ...
        self._last_event_at = now
        self._messages.append(
            BinancePrivateMessage(now, str(payload.get("e", "unknown")), payload)
        )
    ...
    def health(self) -> AdapterHealth:
        age = (
            ...
            if self._last_event_at is None
                ...
                0.0, (pd.Timestamp.now(tz="UTC") - self._last_event_at).total_seconds()
            )
        )
        status = (
            AdapterHealthStatus.HEALTHY
            if self.ready()
            else AdapterHealthStatus.DEGRADED
        )
        return AdapterHealth(
            source="binance_ws_private",
            ts=pd.Timestamp.now(tz="UTC"),
            status=status,
            message=self._last_error,
            metadata={
                "authenticated": self._listen_key is not None,
                "last_event_at": None
                if self._last_event_at is None
                else self._last_event_at.isoformat(),
