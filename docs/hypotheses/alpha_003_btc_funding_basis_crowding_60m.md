...
At a 5m rollover, only the five closed 1m intervals in the preceding bucket are used. The signal is emitted at that rollover decision boundary and the engine's one-bar delay executes it on the following 1m bar. Funding is a latest-source-time backward join with `funding_source_ts <= decision_ts`; every value from the rollover bar is excluded. Funding stress is measured against a past-only fixed 90-day (25,920 decision) distribution. The current completed close is included in trailing 60m return and trailing 6h realized volatility. The 60m target requires twelve complete, contiguous 5m buckets.
...
The strategy is an engine adapter for short entries and explicit close-only 60m exits. The classic engine alone owns fills, costs, delay, sizing, stops, margin and accounting. Validation and test maximum drawdown are independently reconstructed from canonical trade-log net R after filtering to the declared partition. This package grants no capital, order, promotion, or self-approval authority.
