...
CARD_PATH = (
    ROOT
    / "research/hypotheses/cards/f504661fcff41ac7ebe0b37a45139baef2718e0307fc7cfbbdfcc87fe2bcbe44.json"
)
...
def _assignment() -> dict:
    return {
        ...
        "dataset_build_id": "25d9958c-71b6-4c2b-949a-3c0433f70f38",
        ...
        "max_variants": 8,
        "representation_plan": _contract()["representation_plan"],
    }
...
def test_exact_card_plan_and_template_are_independently_bound() -> None:
    ...
    qualification = qualify_card(
        confirm_card(
            first, actor="founder-operator", confirmed_at="2026-09-25T00:00:00Z"
        ),
...
def test_bound_external_review_completes_the_declared_gate(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    card = confirm_card(
        draft_research_card(_assignment(), repository_root=str(ROOT)),
    ...
    monkeypatch.setattr(
        pipeline_module, "governed_review_verified", lambda assignment, item: True
    )
...
def test_complete_rows_enforce_constituents_liquidity_target_and_prior_only_volatility() -> (
    None
):
    panels = _panels(130)
    rows = compile_decision_rows(panels)
    assert rows.loc[100, "prior_only_volatility_source_end_ts"] == rows.loc[
        100, "decision_ts"
    ...
    following = gap_rows.loc[
        gap_rows["decision_ts"] == gap_decision + pd.Timedelta(minutes=15)
    ].iloc[0]
    assert pd.isna(following["btc_return"])
    assert pd.isna(following["eth_return"])


def test_evaluator_fits_train_only_and_retains_all_terminal_outcomes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    panels = _panels()
    params = {
    ...
    negative = liquidity_displacement_evaluation(
        panels,
        params={**params, "response_direction": "reversal"},
        minimum_history=20,
        minimum_extreme_support=1,
        minimum_matched_support=1,
    )
    failed = liquidity_displacement_evaluation(
        panels,
        params=params,
        minimum_history=20,
        minimum_extreme_support=10_000,
        minimum_matched_support=10_000,
    )
    invalid = liquidity_displacement_evaluation(
        _panels(80), params=params, minimum_history=10_000
    )
    ...
    positive = liquidity_displacement_evaluation(
        panels,
        params=params,
        minimum_history=20,
        minimum_extreme_support=1,
...
def test_grid_is_deterministic_and_opens_test_at_most_once() -> None:
    ...
    one = liquidity_displacement_grid_evaluation(
        _panels(), parameter_grid=_contract()["parameter_grid"], **kwargs
    )
    two = liquidity_displacement_grid_evaluation(
        _panels(), parameter_grid=_contract()["parameter_grid"], **kwargs
    )
    assert one == two
...
def test_grid_requires_supported_validation_and_preserves_failure_taxonomy(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    grid = {
        "eth_displacement_tail_percentile": [0.975, 0.99],
        "response_direction": ["continuation"],
    }

    def unsupported_positive(*_args, params, evaluate_test, **_kwargs):
        assert evaluate_test is False
        return {
            "parameters": dict(params),
            "outcome": "failed",
            "validation_directional_effect": 0.01,
        }
    ...
    failed = liquidity_displacement_grid_evaluation({}, parameter_grid=grid)
    assert failed["outcome"] == "failed"
    assert failed["selected_parameters"] is None
    assert failed["test_open_count"] == 0

    calls = 0

    def mixed(*_args, params, evaluate_test, **_kwargs):
        nonlocal calls
        calls += 1
        return {
            "parameters": dict(params),
            "outcome": "invalid" if calls == 1 else "failed",
            "validation_directional_effect": 0.0,
        }

    monkeypatch.setattr(strategy_module, "liquidity_displacement_evaluation", mixed)
    mixed_result = liquidity_displacement_grid_evaluation({}, parameter_grid=grid)
    assert mixed_result["outcome"] == "failed"
    assert mixed_result["test_open_count"] == 0
...
def test_supported_negative_validation_is_not_mislabeled_as_failure() -> None:
    ...
    assert (
        heldout_test_consulted(
            is_liquidity_residual=True,
            liquidity_grid={"test_open_count": 0},
        )
        is False
    )
    assert (
        heldout_test_consulted(
            is_liquidity_residual=True,
            liquidity_grid={"test_open_count": 1},
        )
        is True
    )
...
def _classic_run(frame: pd.DataFrame, output: Path) -> list[dict]:
    ...
    engine = BacktestEngine(
        ...
        execution=ExecutionModel(
            fee_model=FeeModel(maker_fee_bps=6, taker_fee_bps=6),
            slippage_model=SlippageModel(k=0),
            delay_bars=1,
        ),
...
def test_compiler_attachment_and_classic_engine_are_causal_and_deterministic(
    ...
    attached_path = attach_adaptive_features(
