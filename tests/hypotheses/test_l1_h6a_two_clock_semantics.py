...
def _entry(strategy: L1H6AVovGateMeanReversionStrategy) -> tuple[object, int]:
    for i in range(20):
...
def test_stop_and_time_stop_semantics_preserved() -> None:
    strategy = L1H6AVovGateMeanReversionStrategy(timeframe="5m", T_hold=2)
    ...
    strategy2 = L1H6AVovGateMeanReversionStrategy(timeframe="5m", T_hold=2)
