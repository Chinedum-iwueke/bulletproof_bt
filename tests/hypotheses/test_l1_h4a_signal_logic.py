...
def _warm_ready(strategy: L1H4ALiquidityGateMeanReversionStrategy) -> None:
    for i in range(20):
...
def test_long_and_short_signals_respect_liquidity_gate() -> None:
    strategy = L1H4ALiquidityGateMeanReversionStrategy(timeframe="5m", q_liq=0.6, z0=0.8)
    ...
    assert out_long
    assert out_long[0].metadata["liq_gate_t"] is True
    assert out_long[0].metadata["entry_reason"] == "liquidity_gate_fade_long"

    strategy2 = L1H4ALiquidityGateMeanReversionStrategy(timeframe="5m", q_liq=0.6, z0=0.8)
    ...
    assert out_short
    assert out_short[0].metadata["entry_reason"] == "liquidity_gate_fade_short"
